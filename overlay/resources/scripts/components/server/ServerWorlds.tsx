...
export default function ServerWorlds() {
    ...
    return <PageContentBlock title={`${server.name} | ${vt('Mondes')}`} className={form.page}>
        ...
        {error && <p role="alert" className={form.error}>{error}</p>}{result && <div role="status" className={form.success}>{result.message}<Link to={`/server/${server.id}/files#/${result.backup}`}>{vt('Ouvrir les fichiers de récupération')} ↗</Link></div>}
