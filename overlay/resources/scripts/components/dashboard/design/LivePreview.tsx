...
export default function LivePreview({design,path,device,light}:{design:VinusDesignSettings;path:string;device:number;light:boolean}) {
    ...
    useEffect(() => { const resume=()=>{if(document.visibilityState==='visible')send();}; window.addEventListener('pageshow',send); document.addEventListener('visibilitychange',resume); return()=>{window.removeEventListener('pageshow',send);document.removeEventListener('visibilitychange',resume);}; },[design,light]);
