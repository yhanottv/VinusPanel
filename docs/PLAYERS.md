...
- Live data: a compatible **VinusPlayers 1.1.0** companion from the exact matrix below. Bukkit provides acknowledged actions; the native mods provide read-only snapshots, and the same actions are sent to them as vanilla console commands. Folia, Vanilla, proxies, Bedrock and other versions are not automatically supported.
...
The extension includes small bridge JARs built from this repository and their SHA-256 manifest. It includes no Minecraft server or dependency JARs. The installer verifies the checksum locally and after upload, checks the runtime again, and promotes a temporary non-JAR file only while the server is stopped. The file installer itself never changes power state, overwrites an existing detected VinusPlayers JAR, or installs both a plugin and a mod on a hybrid. The explicit post-start activation flow separately orchestrates graceful stop and startup. Bukkit takes precedence on hybrids; a custom hybrid/plugin combination still needs testing by its operator.

The **Versions** and **Modpacks** confirmations install Minecraft alone (`install_players: false`). After success, the panel records a seven-day follow-up in its cache, scoped to the installing account and server, and returns its token to the browser. The global server workspace waits for the first observed **running** state, then opens a separate illustrated VinusPlayers dialog. The workspace restores this invitation after reload, when the server status changes or when the window regains focus, including on another device signed into the same account. The browser also keeps a fallback reminder. Clearing the panel cache can expire the server-side reminder. Installations made before this update do not create a reminder retroactively. The Versions page provides **Discover VinusPlayers** while the server is running to open the presentation again without reinstalling Minecraft. Merely opening the dialog never installs anything. Later, close or Escape dismiss it without any power/file changes. Players remains available with saved data and a manual installation entry point.

The explicit **Stop, install and start again** button explains the brief player disconnection. It requires start/stop permission as well as companion permissions, rechecks availability, gracefully stops a running server, polls until offline for at most two minutes, calls the existing checksum-verified installer, then requests startup only after a newly installed companion succeeds. No force-kill or automatic write retry is used. An existing JAR is preserved and left for operator compatibility review before startup. A failed operation can leave the server stopped; the UI directs the user to Console/Players and never claims a requested startup is already complete. Unknown versions remain informational and cannot trigger companion/power mutations.

There is no unattended companion installation after ordinary server creation/reinstallation. The old event registration is removed and queued jobs without consent do nothing. The former `players_auto_install` setting is unused. The legacy software API still validates its explicit boolean for compatibility, but the current interface defers consent to the separate post-start dialog.
...
For the native mods (Fabric, Forge, NeoForge) the snapshots stay read-only, but the same actions are sent through the server console as vanilla commands (`effect give`, `kill`, `gamemode`, `experience set … levels`, `op`/`deop`, `whitelist add`/`remove`, `ban`/`pardon`). The player name comes from the server's own roster and must match `[A-Za-z0-9_]{1,16}`; values are restricted to fixed lists or bounded integers; the `control.console` permission and a running server are required. These commands have no acknowledgement, so the panel reports the command as sent and the operator should check the player's state afterwards.
