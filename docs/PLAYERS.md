...
The **Versions** and **Modpacks** confirmations install Minecraft alone (`install_players: false`). After success, the browser records a seven-day follow-up scoped to the account and server. The global server workspace waits for the first observed **running** state, then opens a separate illustrated VinusPlayers dialog. It survives navigation/reload in that browser (memory-only fallback when storage is blocked); it is not synchronized across devices. Merely opening the dialog never installs anything. Later, close or Escape dismiss it without any power/file changes. Players remains available with saved data and a manual installation entry point.
