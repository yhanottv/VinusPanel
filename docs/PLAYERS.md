...
After a successful server installation, a delayed Laravel queue job attempts the compatible installation. Leave **Start server when installed** unchecked when creating a Minecraft server so this job can install the bridge before its first start. If auto-start is enabled, the running-server guard skips installation: stop the server and use **Players → Install bridge**. The normal Pterodactyl queue worker must be running. The Versions and Modpacks installers also attempt it after releasing their installation lock. Bridge errors are logged without failing the Minecraft installation. Set `players_auto_install` to `false` in `config/vinuscatalog.php` to disable automatic installation.

For an existing server, open **Players → Install bridge** while stopped, then start it normally. Installation requires `file.read-content`, `file.create`, `file.update` and `control.console`; opening the page alone never installs anything. The theme and extension must both be updated. The theme's Pterodactyl 1.15.1 `EventServiceProvider` overlay registers the installation listener; merge that registration if your installation has custom event listeners.
