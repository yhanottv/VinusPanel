...
Back up the panel, database, game-server files and local configuration before upgrading. Apply the theme with `install.sh`, then update the optional Vinus Catalog Blueprint package to **1.3.0**. The theme installer does not install the extension automatically.
