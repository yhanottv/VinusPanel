...
// Native mod bridges are read-only: their actions go through the server console with vanilla commands.
$console=new ReflectionMethod(PlayerController::class,'consoleCommand');$console->setAccessible(true);
$run=fn($action,$name,$value=null)=>$console->invoke(null,$action,$name,$value);
check($run('heal','Steve')==='effect give Steve minecraft:instant_health 1 1 true');
check($run('feed','Steve')==='effect give Steve minecraft:saturation 2 10 true');
check($run('kill','Steve')==='kill Steve');
check($run('gamemode','Steve','creative')==='gamemode creative Steve');
check($run('experience','Steve',12)==='experience set Steve 12 levels');
check($run('operator','Steve',true)==='op Steve');check($run('operator','Steve',false)==='deop Steve');
check($run('whitelist','Steve',true)==='whitelist add Steve');check($run('whitelist','Steve',false)==='whitelist remove Steve');
check($run('ban','Steve',true)==='ban Steve');check($run('ban','Steve',false)==='pardon Steve');
rejected(fn()=>$run('kill',"Steve\nstop"));
rejected(fn()=>$run('kill','Steve stop'));
rejected(fn()=>$run('kill','a_name_that_is_far_too_long'));
rejected(fn()=>$run('gamemode','Steve','hardcore'));
rejected(fn()=>$run('gamemode','Steve','creative Alex'));
rejected(fn()=>$run('experience','Steve','12'));
rejected(fn()=>$run('experience','Steve',-1));
rejected(fn()=>$run('experience','Steve',10001));
rejected(fn()=>$run('operator','Steve','true'));
rejected(fn()=>$run('say','Steve','hello'));
